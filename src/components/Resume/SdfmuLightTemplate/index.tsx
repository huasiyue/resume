...
const ASSET_BASE = '/sdfmu-light-template';
...
function renderBullets(value?: string, isHtml?: boolean) {
  ...
  if (isHtml) {
    return (
      <div
        className="sdfmu-light-rich-text"
...
const Section: React.FC<SectionProps> = ({
  ...
  return (
    <section className={`sdfmu-light-section ${className || ''}`}>
      <div className="sdfmu-light-section-title">
        <span className="sdfmu-light-section-icon">{icon}</span>
        <span>{title}</span>
      </div>
      <div className="sdfmu-light-section-body">{children}</div>
    </section>
  );
};

export const SdfmuLightTemplate: React.FC<Props> = props => {
  const { value } = props;
  ...
  return (
    <div
      className={`sdfmu-light-resume resume-content ${
        isClassic ? 'sdfmu-light-resume-classic' : 'sdfmu-light-resume-official'
      }`}
    >
      <header className="sdfmu-light-header">
        <div className="sdfmu-light-header-band" />
        <div className="sdfmu-light-logo-panel">
          <img
            className="sdfmu-light-header-logo"
            src={`/sdfmu-template/images/sdfmu-logo-horizontal.png`}
            alt={TEXT.school}
          />
        </div>
        <div className="sdfmu-light-header-caption">
      ...
      {watermarkVisible && (
        <img
          className="sdfmu-light-watermark"
          src={`/sdfmu-template/images/sdfmu-emblem.png`}
          alt=""
        />
      )}

      <main className="sdfmu-light-main">
        {!hiddenMap.profile && (
          <section className="sdfmu-light-profile">
            <div className="sdfmu-light-profile-main">
              <span className="sdfmu-light-profile-kicker">{TEXT.resume}</span>
              <h1>{profile.name || TEXT.resume}</h1>
              {hasText(positionTitle) && <p>{positionTitle}</p>}
              <div className="sdfmu-light-contact-strip">
            ...
            {!value?.avatar?.hidden && (
              <div className="sdfmu-light-photo">
                {value?.avatar?.src ? (
        ...
        {!hiddenMap.profile && profileRows.length > 0 && (
          <Section
            ...
            className="sdfmu-light-profile-section"
          >
            <div className="sdfmu-light-profile-grid">
              {profileRows.map(item => (
                <div className="sdfmu-light-profile-field" key={item.label}>
                  <b>{item.label}</b>
        ...
        {hasEducation && (
          ...
          >
            <div className="sdfmu-light-plain-block">
              {renderHtmlText(
        ...
        {hasResearch && (
          ...
          >
            {researchList.map((research, index) => (
              <div
                className="sdfmu-light-item"
                key={`${research.research_name}-${index}`}
              >
                <div className="sdfmu-light-item-heading">
                  <b>
                    {renderHtmlText(
                ...
                {hasText(research.research_role) && (
                  <div className="sdfmu-light-item-subtitle">
                    {renderHtmlText(
        ...
        {hasProjects && (
          ...
          >
            {projectList.map((project, index) => (
              <div className="sdfmu-light-item" key={`${project.project_name}-${index}`}>
                <div className="sdfmu-light-item-heading">
                  <b>
                    {renderHtmlText(
        ...
        {hasWorkExp && (
          ...
          >
            {workExpList.map((work, index) => (
              <div className="sdfmu-light-item" key={`${work.company_name}-${index}`}>
                <div className="sdfmu-light-item-heading">
                  <b>
        ...
        <div className="sdfmu-light-two-columns">
          {hasSkills && (
            <Section
        ...
        {hasWorkList && (
          ...
          >
            {workList.map((work, index) => (
              <div className="sdfmu-light-item" key={`${work.work_name}-${index}`}>
                <div className="sdfmu-light-item-heading">
      ...
      <div className="sdfmu-light-footer" role="contentinfo">
        <div className="sdfmu-light-footer-title">{TEXT.school}</div>
        <div className="sdfmu-light-footer-en">{TEXT.schoolEn}</div>
      </div>
    </div>
  );
};

export const SdfmuLightClassicTemplate: React.FC<Props> = props => (
  <SdfmuLightTemplate {...props} variant="classic" />
);
